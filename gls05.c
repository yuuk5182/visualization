...
float BoxRotate[NUM_BOXES];     /* 箱の回転角 */
...
void display(void)
{
    ...
    for (i = 0; i < NUM_BOXES; i ++)
    {
        ...
        glColor3f((float)(i/10)/10.0, 
                  (float)(i%10)/10.0, 0);           /* 描画色を設定 */
        glTranslatef((float)(i/10) * 0.3 - 1.5, 
                     (float)(i%10) * 0.3 - 1.5, 0); /* 描画位置を設定 */
        glRotatef(BoxRotate[i], 0.0, 0.0, 1.0);     /* Z軸回転角を設定 */
        glutSolidCube (0.2);                        /* ワイヤーの立方体を描画 */
        glPopMatrix ();                             /* 描画位置を戻す */
    }
...
void timer(int timerID)
{
    ...
    for (i = 0; i < NUM_BOXES; i ++)
    {
        /* 箱の回転角度を1.5度ずつ増加させる */
        BoxRotate[i] += 2.0;
        if (BoxRotate[i] > 360.0) BoxRotate[i] -= 360.0;
    }
...
void myInit (char *windowTitle)
{
    ...
    for (i = 0; i < NUM_BOXES; i ++)
    {
        BoxRotate[i] = ((float)rand() / RAND_MAX) * 360.0;  /* 乱数で0～360度を与える */
    }
}